    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if ticket_quantity <= remaining_tickets:
                ...
                messages.error(
                    request, 'The requested number of tickets exceeds the available tickets.')
        else:
            messages.error(
                request, 'There was an error with your purchase. Please try again.')
    else:
...
def success(request):
    ...
    if purchase_data:
        try:
            ...
            messages.success(
                request, 'Your ticket purchase was successful! Thank you.')
        except Exception as e:
            messages.error(
                request, f'An error occurred while creating your ticket: {e}')
    else:
